...
- [x] 4.8 Convertir `bukeard.com` de landing a **marketplace público** — **código hecho 2026-08-22 noche, EN VIVO desde 2026-08-23**: home con buscador y tarjetas reales, perfil público compartible `/p/:slug`, `/negocios`, `/precios` (plano en [ANALISIS-SITIO-FRESHA.md](ANALISIS-SITIO-FRESHA.md) §5). El Node app se movió de `bukeard.com/app` a `bukeard.com` (raíz) en *Setup Node.js App*, la landing vieja se respaldó como `landing-vieja-backup.html.bak`. Verificado: `/`, `/app`, `/negocios`, `/precios` responden 200 con el contenido nuevo
...
- [x] 4.5 Fotos reales de perfil y portafolio — **hecho 2026-08-25** (adelanta este punto, que seguía marcado Todo): logo del negocio (paso 6 del wizard de registro + editable en cualquier momento) y galería de fotos (`business_photos`, subir/borrar individualmente) desde "Mi negocio" → pestaña "Negocio". **Pendiente:** el perfil público `/p/:slug` todavía no muestra el logo ni la galería — se guardan pero no se ven en ningún lado público todavía (detalle en `docs/PANEL-NEGOCIO-AMPLIACION-2026-08-25.md`)
- [x] 4.14 Ampliación del panel de negocio — **hecho 2026-08-25** (no estaba en el roadmap original): pestaña "Redes sociales" (Instagram/Facebook/TikTok/sitio web), ubicación exacta en el mapa (pin arrastrable, resuelve la imprecisión de geocodificar solo por sector), "Servicios" pasó de solo lectura a editable, y "¿Necesitas ayuda?" (ticket de soporte por correo a hola@bukeard.com, sin tabla ni estado). Detalle completo en `docs/PANEL-NEGOCIO-AMPLIACION-2026-08-25.md`
- [x] 4.15 Panel de administración general — **hecho 2026-08-24/25** (no estaba en el roadmap original): herramienta operativa en `bukeard.com/admin` para administrar la plataforma completa — dashboard, usuarios, negocios, reservas, moderación (verificar cuentas bancarias, borrar comprobantes), métricas de crecimiento/activación/retención, comunicación (WhatsApp/correo con registro de envíos), configuración (banner del sitio, parámetros de reserva). Incluye "Ver panel del negocio" (entrar al panel real de cualquier negocio sin pedirle su PIN) y el arreglo de un bug real: un dueño que entraba desde otro dispositivo no reconocía que ya tenía negocio y lo mandaba a crear uno nuevo. Detalle completo en `docs/PANEL-ADMIN-PENDIENTES-2026-08-25.md` y memoria `bukea_panel_admin`. **Pendiente:** login del panel solo por teléfono+PIN (no Google/Apple todavía), Fase 3 (auditoría, roles de equipo) sin construir
