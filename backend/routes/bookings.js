...
router.post('/', requireAuth, async (req, res) => {
  const { professionalId, serviceId, date, time, paymentMethod } = req.body;
  ...
  try {
    const [result] = await pool.query(
      `INSERT INTO bookings
         (professional_id, service_id, client_user_id, client_name, day_label, time_label,
          appointment_at, duration_min, payment_method)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        professionalId, serviceId, req.user.id, clientName,
